...
import { getSurvivalLabel, getRiskMessage } from "@/logic/survival";
import { analyzeCareer, type AnalyzeResult } from "@/lib/api";
...
export default function CareerAnalysis() {
  ...
  }, [skills, interests, careerGoal, career?.id]);
  ...
  const survivalScore = analysis?.survivalScore ?? 0;
  const missingSkills = analysis?.missingSkills ?? [];
  const recommendation = analysis?.recommendation ?? "Analyzing your inputs...";
  const stressFit = analysis?.stressFit ?? 0;
  const workHoursFit = analysis?.workHoursFit ?? 0;
  const learningCurveFit = analysis?.learningCurveFit ?? 0;

  const survivalInfo = getSurvivalLabel(survivalScore);
  const riskMessage = getRiskMessage(survivalScore, career.title);

  const haveSkills = career.requiredSkills.filter((skill) => !missingSkills.includes(skill));
  const completionPercent = Math.round((haveSkills.length / career.requiredSkills.length) * 100);
      ...
      <div className="flex-1 flex w-full relative">
        ...
        <main className="flex-1 w-full max-w-4xl p-4 md:p-8 lg:p-12 mx-auto space-y-16">
          ...
          <section id="overview" className="space-y-6">
            ...
            <div className="rounded-2xl border border-border/70 bg-background/80 p-4 md:p-6">
              <p className="text-sm uppercase tracking-wide text-muted-foreground mb-2">Career recommendation</p>              <div className="flex flex-wrap gap-3 mb-4">
                <Badge variant="outline" className="text-sm px-3 py-1">
                  Readiness: {analysis?.readinessTime ?? "Estimating..."}
                </Badge>
                <Badge variant="outline" className="text-sm px-3 py-1">
                  Difficulty: {analysis?.difficulty ?? "Estimating..."}
                </Badge>
              </div>              <p className="text-base text-foreground/90">{recommendation}</p>
              {isLoading && <p className="text-sm text-muted-foreground mt-2">Analyzing your inputs...</p>}
          ...
          <section id="skills" className="scroll-mt-20">
            ...
            <Card className="bg-card/50 border-border">
              <CardHeader className="pb-4">
                ...
                <ProgressBar value={completionPercent} colorClass="bg-primary" />
              </CardHeader>
                ...
                <div className="grid md:grid-cols-2 gap-8 mt-6">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {haveSkills.length > 0 ? (
                      haveSkills.map((s, i) => <SkillChip key={i} label={s} variant="success" />)
                    ) : (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {missingSkills.length > 0 ? (
                        missingSkills.map((s, i) => <SkillChip key={i} label={s} variant="danger" />)
                      ) : (
